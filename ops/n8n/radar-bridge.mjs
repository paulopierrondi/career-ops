...
const host = process.env.CAREER_OPS_N8N_BRIDGE_HOST || '127.0.0.1';
const port = Number(process.env.CAREER_OPS_N8N_BRIDGE_PORT || 8765);
...
const runner = `${projectDir}/scripts/n8n-freelance-radar-run.sh`;
...
const server = createServer((req, res) => {
  if (req.method === 'GET' && req.url === '/healthz') {
    return json(res, 200, { ok: true, service: 'career-ops-n8n-radar-bridge' });
  }
...
server.listen(port, host, () => {
  console.log(`career-ops n8n radar bridge listening on http://${host}:${port}`);
});
